...
export default function Spin3Screen({ onNavigate, winData }) {
  ...
  useEffect(() => {
    const loadCaseData = async () => {
      try {
        ...
        const response = await casesApi.getCaseById(2);
  ...
  useEffect(() => {
    ...
    console.log('🔄 Spin3Screen инициализация');
    ...
    const initializeData = () => {
      ...
      // ПРОВЕРКА: winData может быть объектом с ключом winData или просто winData
      ...
      // РЕАЛЬНЫЙ РЕЖИМ БЕЗ winData (ошибка)
        ...
        setTimeout(() => onNavigate('card3'), 100);
        return;
      }
